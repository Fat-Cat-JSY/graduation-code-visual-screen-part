	...
	<div>
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'/'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/taobaoinfoforecast?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
			...
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						...
						<div class="text "  >{{detail.prosheng}}</div>
					</div>
					<div class="item">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('taobaoinfoforecast','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('taobaoinfoforecast','删除')" @click="delClick">删除</el-button>
						<el-button v-if="btnAuth('taobaoinfoforecast','预测')" class="forecastBtn" type="success" @click="forecastClick()">
							 预测 						</el-button>
						<el-button class="editBtn" @click="allchapterClick()" type="warning" v-if="btnAuth('taobaoinfoforecast','章节管理')">章节管理</el-button>
					</div>
				</div>
			</div>
		...
		methods: {
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/taobaoinfoforecast', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/taobaoinfoforecastAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此预测信息？') .then(_ => {
					this.$http.post('taobaoinfoforecast/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
			...
			forecastClick(row) {
				this.$confirm(`是否进行数据预测?`, "提示", {
					...
					this.$http.post('taobaoinfoforecast/forecast',brr).then(res=>{
			...
			allchapterClick (){
				...
				this.$router.push({path: '/index/chaptertaobaoinfoforecast', query: params});
			},
			// 获取uuid
			getUUID() {
