...
from backend.apps.core.views import config_v
from backend.apps.analytics.views import schema_v
from backend.apps.analytics.views import spark_v
...
# core app的views目录路径
coreDir = os.path.dirname(__file__)
viewsDir = os.path.join(coreDir, "views")

# 过滤文件的列表（现在都在 views 子目录下，不需要排除旧文件）
excludeList = []

# 循环views目录下的_v.py文件
view_tuple = set()
for i in os.listdir(viewsDir):
    if i not in excludeList and i[-5:] == "_v.py":
        viewName = i[:-3]  # 去掉.py后缀字符串
        view_tuple.add("from backend.apps.core.views import {}".format(viewName))
...
for i in os.listdir(viewsDir):
    if i not in excludeList and i[-5:] == "_v.py":
        ...
        if tableName.lower() == "taobaoinfo":
            urlpatterns.extend(
                [
                    path(r'{}/trainRecommend'.format(tableName.lower()),
                         eval("{}_v.{}_trainRecommend".format(tableName.capitalize(), tableName.lower()))),
                ]
            )
        if tableName.lower() == "taobaoinfo":
            urlpatterns.extend(
        ...
        if tableName.lower() == "taobaoinfo":
            urlpatterns.extend(
                [
                    path(r'{}/cleanse'.format(tableName.lower()),
                         eval("{}_v.{}_cleanse".format(tableName.capitalize(), tableName.lower())))
                ]
            )
        if tableName.lower() == "taobaoinfo":
            urlpatterns.extend(
        ...
        # 预测
        if tableName.lower() == "taobaoinfoforecast":
            urlpatterns.extend(
                [
                    path(r'{}/forecastimgs'.format(tableName.lower()),
                         eval("{}_v.{}_forecastimgs".format(tableName.capitalize(), tableName.lower()))),
                    path(r'{}/forecast'.format(tableName.lower()),
                         eval("{}_v.{}_forecast".format(tableName.capitalize(), tableName.lower()))),
                ]
            )
