...
spark_dir = "D:/DevSoft/spark-3.1.2"
python_dir = "D:/Python377"
...
def upload_csv():
    ...
    with open(yonghu_path, 'w', newline='', encoding='utf-8') as csvfile:
        ...
        for row in yonghu_column_info:
            writer.writerow(row)
    # 上传映射文件
    yonghu_hdfs_csv_path = f'/input/yonghu.csv'
    yonghu_local_csv_path = os.path.join(parent_directory,"yonghu.csv")
    # 删除已有的数据
    if hadoop_client.status(yonghu_hdfs_csv_path, strict=False):
        hadoop_client.delete(yonghu_hdfs_csv_path, recursive=True)
    hadoop_client.upload(yonghu_hdfs_csv_path, yonghu_local_csv_path)
    cursor.execute("SELECT * FROM taobaoinfo")
    taobaoinfo_column_info = cursor.fetchall()
    # 将数据写入 CSV 文件
    taobaoinfo_path = os.path.join(parent_directory, "taobaoinfo.csv")
    with open(taobaoinfo_path, 'w', newline='', encoding='utf-8') as csvfile:
        writer = csv.writer(csvfile)
        # 写入表头
        writer.writerow([desc[0] for desc in cursor.description])
        # 写入数据行
        for row in taobaoinfo_column_info:
            writer.writerow(row)
    # 上传映射文件
    taobaoinfo_hdfs_csv_path = f'/input/taobaoinfo.csv'
    taobaoinfo_local_csv_path = os.path.join(parent_directory,"taobaoinfo.csv")
    # 删除已有的数据
    if hadoop_client.status(taobaoinfo_hdfs_csv_path, strict=False):
        hadoop_client.delete(taobaoinfo_hdfs_csv_path, recursive=True)
    hadoop_client.upload(taobaoinfo_hdfs_csv_path, taobaoinfo_local_csv_path)
    # 上传group文件
...
def send_cmd():
    ...
    f'''{spark_dir}/bin/spark-submit.cmd \
        ...
        yonghu--yonghuxingming--nianling--
    ''',
    f'''{spark_dir}/bin/spark-submit.cmd \
        --master spark://{master_host}:7077 \
        --conf "spark.pyspark.driver.python={python_dir}/python.exe" \
